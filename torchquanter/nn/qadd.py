...
class QAdd(QModule):
    ...
    def quantize_inference(self, x1, x2, mode=None):
        ...
        if mode is None:
            x1 = broadcast_dim_as(self.M1, x1, dim=1) * x1
            x2 = broadcast_dim_as(self.M2, x2, dim=1) * x2
            out = x1 + x2
            out.round_() 
        elif mode == 'cmsis_nn':
            multiplier1, shift1 = approximate_float(self.M1)
            multiplier2, shift2 = approximate_float(self.M2)
            x1 = (x1 * broadcast_dim_as(multiplier1, x1, dim=1)) >> (31 - broadcast_dim_as(shift1, x1, dim=1))
            x2 = (x2 * broadcast_dim_as(multiplier2, x2, dim=1)) >> (31 - broadcast_dim_as(shift2, x2, dim=1))
            out = x1 + x2
        else:
